  ...
  const insertImage = useCallback(
    async (file) => {
      ...
      if (sizeMB > maxImageSizeMB) {
        alert(
          `حجم الصورة ${sizeMB.toFixed(1)}MB — الحد الأقصى ${maxImageSizeMB}MB`
        );
        return;
      }
...
export default function CompetitionsQuestions({ value = [], onChange, uploadImage }) {
  ...
  const upsertQuestion = () => {
    ...
    if (qType === "mcq") {
      if (mcqSubType === "general") {
        ...
        if (options.some((o) => !o.textHtml?.trim()))
          return alert("كل الخيارات يجب أن تحتوي نصًا.");
        payload = {
          id: editingId || uid(),
          type: "mcq",
          mcqSubType: "general",
        ...
        if (!chemEquationLatex?.trim())
          return alert("أدخل نص المعادلة/المسألة.");
        ...
        if (!answers.some((a) => a.correct))
          return alert("اختر إجابة صحيحة واحدة على الأقل.");

        const correctLatex = answers
          .filter((a) => a.correct)
          .map((a) => a.latex);
    ...
    const next = sections.map((s) => {
      ...
      return {
        ...
        questions:
          exists >= 0
            ? list.map((q) => (q.id === payload.id ? payload : q))
            : [payload, ...list],
      };
    });
    pushChange(next);
      ...
      if (q.mcqSubType === "general") {
        ...
        const ansObjs = Array.isArray(q.answers)
          ? q.answers.map((a, i) => ({
              ...
              explanationHtml:
                typeof a === "object" && a?.explanationHtml
                  ? a.explanationHtml
                  : "",
              isCorrect:
  ...
  const addChemAnswer = () =>
    setChemAnswers((arr) => [
      ...arr,
      { id: uid(), latex: "", explanationHtml: "", isCorrect: false },
    ]);
              ...
              <Segmented
                ...
                options={[
                  ...
                  },
                  {
                    label: (
                      <div className="flex items-center gap-2 px-4 py-2">
                        <QuestionCircleOutlined className="w-4 h-4" />
                        <span>اكمل</span>
                      </div>
                    ),
                    value: "complete",
                              ...
                              {row.opts.map((o, oi) => (
                                <OptionCard
                                  ...
                                  onDelete={() => {
                                    ...
                                    next[i] = {
                                      ...next[i],
                                      opts: next[i].opts.filter(
                                        (_, idx) => idx !== oi
                                      ),
                                      correctIndex:
                                        next[i].correctIndex >= oi
                                          ? Math.max(
                                              0,
                                              next[i].correctIndex - 1
                                            )
                                          : next[i].correctIndex,
                                    };
                                    setPassageQuestions(next);
              ...
              {mcqSubType === "chemical" && (
                <div className="space-y-8">
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center justify-between">
                      <Text strong className="text-base text-gray-800">
                        الإجابات (يمكن أكثر من إجابة صحيحة)
                      </Text>
                      <Button
                        icon={<PlusOutlined />}
                        className="rounded-xl"
                        onClick={addChemAnswer}
                      >
                        إضافة إجابة
                      </Button>
                    </div>
                    ...
                    <div className="space-y-3">
                      {chemAnswers.map((ans, idx) => (
                        <div
                          key={ans.id}
                          className="rounded-xl border p-3 bg-white space-y-3"
                        >
                          <div className="flex items-center justify-between">
                            ...
                            <div className="flex items-center gap-2">
                              <Checkbox
                                ...
                                onChange={(e) =>
                                  toggleChemAnswerCorrect(idx, e.target.checked)
                                }
                              >
                                إجابة صحيحة
                              </Checkbox>
                          ...
                          <div className="space-y-2">
                            <Text className="block text-xs font-semibold text-gray-600">
                              صيغة الإجابة
                            </Text>
                            <MathFieldInput
                              value={ans.latex}
                              onChange={(latex) =>
                                updateChemAnswerLatex(idx, latex)
                              }
                  ...
                  >
                    <div className="grid grid-cols-2 gap-4">
                      <Radio value="true" className="!block">
                        <Card
                          className={`border-2 p-4 text-center transition-all ${
                            tfCorrect ? "border-green-500 bg-green-50" : "border-gray-200"
                          }`}
                        >
                      ...
                      <Radio value="false" className="!block">
                        <Card
                          className={`border-2 p-4 text-center transition-all ${
                            !tfCorrect ? "border-red-500 bg-red-50" : "border-gray-200"
                          }`}
                        >
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <Button
                    icon={<PlusOutlined />}
                    onClick={() =>
                      setCompleteAnswers((p) => [...p, { id: uid(), answer: "" }])
                    }
                    className="rounded-xl"
                  >
                    إضافة إجابة
                  ...
                  {completeAnswers.map((a, i) => (
                    <div key={a.id} className="flex items-center gap-2">
                      <Input
                        ...
                        onChange={(e) =>
                          setCompleteAnswers((prev) =>
                            prev.map((x) =>
                              x.id === a.id ? { ...x, answer: e.target.value } : x
                            )
                          )
                        }
                      ...
                      <Button
                        ...
                        icon={<DeleteOutlined />}
                        onClick={() =>
                          setCompleteAnswers((p) => p.filter((x) => x.id !== a.id))
                        }
                      />
                    </div>
                  ))}
        ...
        title={
          <div className="flex items-center gap-2">
            ...
            <Tag color="blue" className="ms-2">
              {activeSection?.questions?.length || 0} سؤال
            </Tag>
          </div>
        }
      ...
      >
        {!activeSection || (activeSection.questions || []).length === 0 ? (
          <div className="text-gray-500 text-center py-8">
            لا توجد أسئلة بعد في هذا القسم.
          </div>
        ) : (
          <div className="space-y-6">
              ...
              <div key={q.id} className="rounded-xl border p-4 bg-white">
                <div className="flex items-start justify-between gap-3">
                      ...
                      <div className="space-y-3">
                        ...
                        {(() => {
                          ...
                          return list.map((a, i) => (
                            <div
                              key={i}
                              className="border rounded-lg p-2 bg-gray-50 mb-2"
                            >
                    ...
                    {q.type === "trueFalse" && (
                      <>
                        ...
                        <div className="mt-1 text-sm">
                          ...
                          <b
                            className={
                              q.correct ? "text-emerald-700" : "text-rose-700"
                            }
                          >
                            {q.correct ? "صحيح" : "خطأ"}
                          </b>
                  ...
                  <Space>
                    ...
                    <Button size="small" danger onClick={() => removeQuestion(q.id)}>
                      حذف
                    </Button>
                  </Space>
                </div>
