...
//       })
//     );
//   }, [dispatch, activeTab, category_id, page]);
...
const SubjectsManagementPage = () => {
  ...
  const activeTab = useMemo(() => {
    // If there's a category_id in URL, use it
    if (category_id) {
      return category_id;
    }
    
    // Otherwise, get the first category from the list
    const cats = all_courses_categories_list?.data?.message?.data;
  ...
  const handleTabChange = (tabId) => {
    // Update URL with new category_id
        ...
        <PagesHeader
          ...
          extra={
            <div className="flex items-center gap-4 gap-reverse">
              <Button
                onClick={() => {
                  // Pass the active category to the add page
