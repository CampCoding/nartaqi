...
export default function AddCourseSourceResource({
  ...
  // Custom validator to ensure at least one resource exists
  const validateResources = () => {
    const values = form.getFieldsValue();
    const telegram = values?.resources?.telegram || "";
    const whatsapp = values?.resources?.whatsapp || "";
    
    // Get current files count from Redux
    const resources = all_resources_list?.data?.message?.resource || [];
    const hasLinks = telegram.trim() !== "" || whatsapp.trim() !== "";
    const hasFiles = resources.length > 0;
    
    if (!hasLinks && !hasFiles) {
      return Promise.reject(new Error('يجب إضافة رابط على الأقل أو ملف'));
    }
  ...
  const handleSaveAndContinue = async () => {
    try {
      // First validate that at least one resource exists
      await validateResources();
      
      // Then validate form fields (links validation only)
      await form.validateFields([['resources', 'telegram'], ['resources', 'whatsapp']]);
      ...
      console.error("Error saving resources:", err);
      if (err.message === 'يجب إضافة رابط على الأقل أو ملف') {
        message.error(err.message);
      } else if (err.errorFields && err.errorFields.some(field => 
        field.name[0] === 'resources' && 
        (field.name[1] === 'telegram' || field.name[1] === 'whatsapp')
      )) {
        message.error("يرجى إدخال روابط صحيحة");
      } else {
        message.error("حدث خطأ أثناء حفظ المصادر");
      }
  ...
  return (
    <div className="space-y-8" dir="rtl">
      <Form form={form} layout="vertical">
        {/* Hidden form item for overall resources validation */}
        <Form.Item name="resources" rules={[{ validator: validateResources }]} noStyle>
          <div className="bg-gradient-to-r from-purple-50 to-indigo-50 rounded-2xl p-6 border border-purple-200">
            <div className="flex items-center justify-between gap-3 flex-wrap">
              <h3 className="text-xl font-bold text-gray-800 mb-4 sm:mb-0 flex items-center gap-3">
                <FolderOutlined className="text-purple-600" />
