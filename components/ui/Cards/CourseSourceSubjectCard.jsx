...
const CourseSourceSubjectCard = ({
  ...
  useEffect(() => {
    ...
    setSelectedTeacher(teachers_list?.data?.message?.find(item => item?.id == subject?.teacher_id))
  }  ,[subject , teachers_list])
      ...
      >
        ...
        <div className="text-black self-stretch p-3 flex flex-col justify-start items-start gap-3">
          ...
          <div className="flex w-full justify-between items-center">
            ...
            <div className="flex items-center gap-[5px]">
             {selectedTeacher?.image_url &&  <img
                className="w-6 h-6 rounded-xl"
                src={selectedTeacher?.image_url}
                alt="instructor"
              />}
              <div className="text-text text-[10px] font-medium">
                المدرس: {selectedTeacher?.name || "—"}
              </div>
            </div>
          </div>
