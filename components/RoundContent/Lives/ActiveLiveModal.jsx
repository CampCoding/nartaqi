...
import { handleActiveLive, handleDeleteLive, handleGetAllLives } from '../../../lib/features/livesSlice';
import { handleGetAllRoundContent } from '../../../lib/features/roundContentSlice';


const { Text } = Typography;

export default function ActiveLiveModal({ open, setOpen, rowData , id  , round_id , selectedLesson}) {
  const dispatch = useDispatch();
  const {active_live_loading} = useSelector(state => state?.lives);

  const handleActivateLive = () => {
     const body = { id: rowData?.id, active: rowData?.active == "1" ? "0" : "1" };
  ...
  const contentTitle = rowData?.title || 'هذا البث المباشر';
  ...
  const modalFooter = (
    <div className='flex justify-start space-x-2 space-x-reverse pt-4'>
      <Button
        ...
        loading={active_live_loading}
        className='rounded-md px-6'
        icon={<EyeOutlined />}
      >
         {rowData?.active == "1" ? "إلغاء تنشيط":"تنشيط"}
      </Button>
      <Button
        key="back"
        onClick={() => setOpen(false)}
        disabled={active_live_loading}
        className='rounded-md px-6'
      >
        إلغاء
      </Button>
    ...
    >
      <div className='flex flex-col gap-4 mt-4 text-right'>
        <Text strong style={{ fontSize: '1.1rem' }} className='text-gray-800'>
          هل أنت متأكد من {rowData?.active == "1"? "إلغاء تنشيط":"تنشيط"} البث المباشر : <Text mark>{contentTitle}</Text>؟
        </Text>
