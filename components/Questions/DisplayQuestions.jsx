...
export default function DisplayQuestions({
  ...
  const toggleQuestion = (id, type) => {
    ...
    if (editingQuestionId === id && !prev[key]) {
      cancelEditing();
    }
  };
                  ...
                  <div className={`p-4 rounded-lg ${item.correctAnswer ? 'bg-green-50 border border-green-200' : 'bg-red-50 border border-red-200'}`}>
                    ...
                    {item.explanation && (
                      <div className={`mt-3 pt-3 border-t border-gray-200 ${textWrapClass}`}>
                        <div className="text-sm font-medium text-gray-700 mb-1">الشرح:</div>
                ...
                <div className="space-y-2">
                  {item.options?.map((opt, oIdx) => (
                    ...
                    >
                      ...
                      {opt.explanation && (
                        <div className={`mt-1 text-sm text-gray-600 pr-6 ${textWrapClass}`}>
                          <span className="font-medium">الشرح:</span>
                          <span dangerouslySetInnerHTML={{ __html: opt.explanation }} />
                        </div>
                      )}
                    ...
                    >
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          ...
                          <div className={`text-sm text-gray-500 ${textWrapClass}`}>
                            {q.type === "mcq"
