...
//           title: nameHtml,
//           description: descHtml,
//           time_if_free:(data?.type =="intern") ? time :   null,
//           questions: [],
//         };
...
//              {data?.type == "intern" && <div className="flex flex-col gap-2">
//                 <label className="text-gray-500 text-lg">الوقت</label>
//                 <input type="text" onChange={(e) => setTime(e?.target?.value)} value={time} className="w-full border border-gray-300 p-2 rounded-md"/>
//               </div>}
//             </div>
  ...
  const handleSaveSection = () => {
    ...
    if (isEditing && editingSection) {
      const updatedSection = {
        ...
        time_if_free: examInfoData?.type == "intern" ? time : null,
      };
          ...
          <div className="mt-3 space-y-4">
            ...
            <div className="flex gap-3 justify-end">
              ...
              <button
                ...
                disabled={
                  !nameHtml?.replace(/<p>|<\/p>/g, "")?.trim() ||
                  (isInternType && !time) || // Disable if intern type and no time
                  add_exam_section_loading ||
                  update_exam_section_loading
                }
                title={
                  !nameHtml?.replace(/<p>|<\/p>/g, "")?.trim()
                    ? "اسم القسم مطلوب"
                    : isInternType && !time
                      ? "الوقت مطلوب للاختبارات الداخلية"
                      : undefined
                }
              >
                {getButtonText()}
                ...
                localSections.map((section) => (
                  ...
                  >
                    <div className="flex-1">
                      ...
                      {isInternType && section.time_if_free && (
