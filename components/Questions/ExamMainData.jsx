...
const VideoCard = ({
  ...
  setRowData,
  isExamVideo = false,
  setOpenEditExamVideo,
  setOpenDeleteExamVideo,
  setSelectedExamVideo,
}) => {
    ...
    >
      ...
      {openExamSection && get_exam_sections_list?.data?.message?.length && (
        <div className="flex gap-4 rtl:flex-row-reverse">
          {/* Add Video Button */}
          <button
            onClick={() => setOpenExamVideoModal(true)}
            className="flex items-center gap-2.5 px-6 py-3 bg-gradient-to-r from-purple-600 to-pink-600 text-white font-medium rounded-xl shadow-lg hover:shadow-xl hover:from-purple-700 hover:to-pink-700 transform hover:-translate-y-0.5 transition-all duration-200"
          >
            <FileVideo size={20} />
          ...
          {/* Add File Button */}
          <button
            onClick={() => setOpenExamPdfModal(true)}
            className="flex items-center gap-2.5 px-6 py-3 bg-gradient-to-r from-blue-600 to-cyan-600 text-white font-medium rounded-xl shadow-lg hover:shadow-xl hover:from-blue-700 hover:to-cyan-700 transform hover:-translate-y-0.5 transition-all duration-200"
          >
            <FilePlus size={20} />
            <span>إضافة ملف</span>
          </button>
      ...
      {/* {all_exam_data_list?.data?.message} */}

      {/* Videos Section */}
      {all_exam_data_list?.data?.message?.exam_pdfs?.length > 0 || all_exam_data_list?.data?.message?.exam_videos?.length > 0 ?
        <div className="mb-6">
          <div className="flex justify-between items-center mb-3">
            <h5 className="text-md font-semibold text-gray-700 flex items-center">
              ...
              فيديوهات الاختبار (
              {all_exam_data_list?.data?.message?.exam_videos?.length || 0})
            </h5>
          </div>

          {all_exam_data_list?.data?.message?.exam_videos?.length > 0 ? (
            <div className="space-y-2">
              {all_exam_data_list?.data?.message?.exam_videos?.map((video) => (
                <VideoCard
                  ...
                  video={video}
                  isExamVideo={true}
                  setOpenDeleteVideo={setOpenDeleteExamVideo}
                  setOpenDeleteExamVideo={setOpenDeleteExamVideo}
                  setOpenEditExamVideo={setOpenEditExamVideo}
                  setSelectedExamVideo={setSelectedExamVideo}
                />
              ))}
            </div>
          ) : (
          ...
          <div className="mb-6">
            <div className="flex justify-between items-center mb-3">
              <h5 className="text-md font-semibold text-gray-700 flex items-center">
                ...
                ملفات PDF (
                {all_exam_data_list?.data?.message?.exam_pdfs?.length || 0})
              </h5>
            </div>

            {all_exam_data_list?.data?.message?.exam_pdfs?.length > 0 ? (
              <div className="space-y-2">
                {all_exam_data_list?.data?.message?.exam_pdfs?.map((pdf) => (
                  <PdfCard
                    setRowData={setPdfRowData}
                    rowData={pdfRowData}
                    key={pdf.id}
                    pdf={pdf}
                    setSelectedExamPdf={setSelectedExamPdf}
                    setOpenEditExamPdf={setOpenEditExamPdf}
                    setOpenDeleteExamPdf={setOpenDeleteExamPdf}
                  />
                ))}
              </div>
            ) : (
